...
function normalizeReportList(values, allowed) {
  const allowedList = Array.isArray(allowed) ? allowed : [];
  const allowedMap = new Map(allowedList.map(value => [normalizeOptionValue(value), value]));
  const raw = Array.isArray(values) ? values : String(values || '').split(',');
  const selected = raw
    .map(value => allowedMap.get(normalizeOptionValue(value)))
    .filter(Boolean);
  return selected.length ? [...new Set(selected)] : [...allowedList];
}

function metaTipo(origem) {
  const normalized = normalizeOptionValue(origem);
...
async function reportOperationsContext(operationFilter = '') {
  await ensureDefaultOperacoes();
  const grouped = await configOptionsGrouped();
  const productList = Array.isArray(grouped.produto) ? grouped.produto : DEFAULT_CONFIG_OPTIONS.produto;
  const destinationList = Array.isArray(grouped.destino) ? grouped.destino : DEFAULT_CONFIG_OPTIONS.destino;
  const docs = sortDocs(await selectDocs(TABLES.operacoes), { ordem: 1, origem: 1 });
  const operations = docs.map((op, index) => {
    const origem = normalizeUniqueValue(op.origem || `OPERACAO ${index + 1}`);
    const produtos = normalizeReportList(op.produtos, productList);
    return {
      ...op,
      origem,
      metaTipo: op.metaTipo || metaTipo(origem),
      produtos,
      resumoProdutos: normalizeReportList(op.resumoProdutos, productList),
      resumoDestinos: normalizeReportList(op.resumoDestinos, destinationList),
      ordem: Number(op.ordem) || index + 1
    };
  });
  const normalizedFilter = normalizeOptionValue(operationFilter);
  return normalizedFilter
    ? operations.filter(op => normalizeOptionValue(op.origem) === normalizedFilter)
    : operations;
}
...
function reportMetaForProductDestination(metas, date, operation, destino, produto) {
  ...
  const metaDoc = metas.find(meta =>
    ...
    meta.tipo === tipo &&
    normalizeOptionValue(meta.produto) === normalizeOptionValue(produto)
  );
  return metaDoc ? parseNumber(metaDoc.valor) : 0;
}

function reportMetaForDestination(metas, date, operation, destino) {
  return operation.resumoProdutos.reduce((sum, produto) => (
    sum + reportMetaForProductDestination(metas, date, operation, destino, produto)
  ), 0);
}

function filterReportRows(rows, operations) {
  const byOrigin = Object.fromEntries(operations.map(op => [
    normalizeOptionValue(op.origem),
    new Set(op.resumoProdutos.map(normalizeOptionValue))
  ]));
  return rows.filter(row => {
    const products = byOrigin[normalizeOptionValue(row.origem)];
    return products && products.has(normalizeOptionValue(row.produto));
  });
}
...
function buildReportSummaryCards(rows, metas, dates, operations) {
  return operations.map(operation => {
    const operationRows = rows.filter(row =>
      normalizeOptionValue(row.origem) === normalizeOptionValue(operation.origem) &&
      operation.resumoProdutos.map(normalizeOptionValue).includes(normalizeOptionValue(row.produto))
    );
    ...
    const columns = operation.resumoDestinos.map(destino => {
...
async function buildReportsDataForExport(filters) {
  const { start, end, operation } = filters;
  const operations = await reportOperationsContext(operation);
  const dates = datesBetween(start, end);
  const viagens = (await selectDocs(TABLES.viagens))
    .filter(doc => String(doc.data || '') >= start && String(doc.data || '') <= end);
  const metas = await selectDocs(TABLES.metas);
  ...
  dates.forEach(date => {
    operations.forEach(op => {
      op.resumoDestinos.forEach(destino => {
...
function styleReportSheet(sheet) {
  ...
  sheet.eachRow((row, rowNumber) => {
    row.eachCell(cell => {
      cell.border = {
        top: { style: 'thin', color: { argb: 'FFD5DEE8' } },
        left: { style: 'thin', color: { argb: 'FFD5DEE8' } },
        bottom: { style: 'thin', color: { argb: 'FFD5DEE8' } },
        right: { style: 'thin', color: { argb: 'FFD5DEE8' } }
      };
      if (rowNumber === 1 || String(cell.value || '').startsWith('Relatorio')) {
        cell.font = { name: 'Arial', size: rowNumber === 1 ? 14 : 10, bold: true };
      }
    });
  });
}

function buildReportsWorkbook(report) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'FrotaSys';
  workbook.created = new Date();
  ...
  summary.addRow(['Relatorio de Operacoes']);
  summary.addRow(['Periodo', `${formatDateBR(report.filters.start)} a ${formatDateBR(report.filters.end)}`]);
  summary.addRow(['Operacao', report.filters.operation || 'Todas as operacoes']);
  summary.addRow(['Total viagens', report.rows.length]);
  summary.addRow(['Meta total', formatPeso(report.metaTotal), 'Carregado', formatPeso(report.loadedTotal), '% Atendimento', `${Math.round(report.atendimento)}%`]);
  summary.addRow([]);
  summary.addRow(['Modalidade', 'Registros']);
  report.typeCounts.forEach(item => summary.addRow([item.label, item.value]));
  summary.addRow([]);
  summary.addRow(['Operacao', 'Meta', 'Faturado', 'Agenciado', 'Total', 'Falta', '% Meta']);
  report.summaryCards.forEach(card => {
    summary.addRow([
      card.origem,
      formatPeso(card.totals.meta),
      formatPeso(card.totals.fat),
      formatPeso(card.totals.agenc),
      formatPeso(card.totals.total),
      formatPeso(card.totals.falta),
      `${card.percent}%`
    ]);
  });
  summary.addRow([]);
  summary.addRow(['Destino', 'Meta', 'Carregado', '% Atendimento']);
  report.destinos.forEach(item => summary.addRow([item.destino, formatPeso(item.meta), formatPeso(item.loaded), `${Math.round(item.percent)}%`]));
  styleReportSheet(summary);

  const viagensSheet = workbook.addWorksheet('Viagens');
  viagensSheet.addRow(VIAGENS_EXPORT_COLUMNS.map(column => column.header));
  sortDocs(report.rows, { data: 1, createdAt: 1 }).forEach(viagem => {
    viagensSheet.addRow(VIAGENS_EXPORT_COLUMNS.map(column => exportCellValue(viagem, column.key)));
  });
  styleExportWorksheet(viagensSheet);

  const daily = workbook.addWorksheet('Evolucao');
  daily.columns = [
    { key: 'data', width: 16 },
    { key: 'meta', width: 18 },
    { key: 'carregado', width: 18 }
  ];
  daily.addRow(['Data', 'Meta', 'Carregado']);
  report.daily.forEach(item => daily.addRow([formatDateBR(item.date), formatPeso(item.meta), formatPeso(item.loaded)]));
  styleReportSheet(daily);
...
function buildSimplePdf(lines) {
  ...
  pageLines.forEach(page => {
    const content = [
      ...
      '/F1 12 Tf',
      '40 800 Td',
      ...page.map((line, index) => `${index ? '0 -17 Td ' : ''}(${pdfSafeText(line)}) Tj`),
      'ET'
    ].join('\n');
...
function buildReportsPdf(report) {
  const lines = [
    'Relatorio de Operacoes',
    `Periodo: ${formatDateBR(report.filters.start)} a ${formatDateBR(report.filters.end)}`,
    `Operacao: ${report.filters.operation || 'Todas as operacoes'}`,
    `Total de viagens: ${report.rows.length}`,
    `Meta total: ${formatPeso(report.metaTotal)} | Carregado: ${formatPeso(report.loadedTotal)} | Atendimento: ${Math.round(report.atendimento)}%`,
    '',
    'Modalidade'
  ];
  report.typeCounts.forEach(item => lines.push(`${item.label}: ${item.value} registro${item.value === 1 ? '' : 's'}`));
  lines.push('', 'Resumo Consolidado', 'Operacao | Meta | Faturado | Agenciado | Total | Falta | % Meta');
  report.summaryCards.forEach(card => {
    lines.push(`${card.origem} | ${formatPeso(card.totals.meta)} | ${formatPeso(card.totals.fat)} | ${formatPeso(card.totals.agenc)} | ${formatPeso(card.totals.total)} | ${formatPeso(card.totals.falta)} | ${card.percent}%`);
  });
  lines.push('', 'Meta por destino', 'Destino | Meta | Carregado | % Atendimento');
  report.destinos.forEach(item => {
    lines.push(`${item.destino} | ${formatPeso(item.meta)} | ${formatPeso(item.loaded)} | ${Math.round(item.percent)}%`);
  });
...
app.get('/api/reports/export', async (req, res) => {
  try {
    const start = String(req.query.start || req.query.inicio || '').trim();
    const end = String(req.query.end || req.query.fim || '').trim();
    ...
    if (datesBetween(start, end).length > 366) {
      return res.status(400).json({ error: 'Selecione um período de até 366 dias.' });
    }
    ...
    const report = await buildReportsDataForExport({ start, end, operation });
    const suffix = `${formatDateSheetName(start)}_${formatDateSheetName(end)}`;

    if (format === 'pdf') {
      const buffer = buildReportsPdf(report);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="relatorios_${suffix}.pdf"`);
      res.setHeader('Content-Length', buffer.length);
      return res.send(buffer);
    }

    const workbook = buildReportsWorkbook(report);
    const buffer = await workbook.xlsx.writeBuffer();
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="relatorios_${suffix}.xlsx"`);
    res.setHeader('Content-Length', buffer.length);
    res.send(Buffer.from(buffer));
  } catch (e) {
    res.status(500).json({ error: e.message });
